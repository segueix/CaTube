...
// --- SEGUEIX SHARING LOGIC ---
// 1. Function to share a playlist (POST)
function shareSegueixPlaylist(playlistName) {
    const storedData = JSON.parse(localStorage.getItem(playlistName) || '[]');
    if (storedData.length === 0) {
        return alert('This playlist is empty.');
    }
    ...
    // Extract IDs
    const ids = storedData.map(v => v.id).join(',');
...
// 2. Listener to load shared playlist (GET)
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (params.has('list')) {
        ...
        if (mainContent) {
            mainContent.innerHTML = '<div style="text-align:center;padding:50px">🔄 Loading shared playlist...';
        }
        ...
        fetch(`${SEGUEIX_API_URL}?id=${listId}`)
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    alert('Playlist not found or deleted.');
...
// 3. Render the shared view
function renderSharedPlaylist(name, stringIds) {
    const ids = stringIds.split(',');
    // Use the global feed generated by update_feed.js
    const videoPool = window.FEED_UPDATES || [];
    const videos = ids.map(id => videoPool.find(v => v.id === id)).filter(Boolean);
    ...
    mainContent.innerHTML = `
        <header style='padding:20px; display:flex; justify-content:space-between; align-items:center;'>
            ...
            <div>
                <button onclick="saveImportedPlaylist('${name}', '${stringIds}')" style='padding:10px; background:var(--color-primary); border:none; color:white; border-radius:5px; cursor:pointer;'>💾 Save Playlist</button>
                <a href='${window.location.pathname}' style='margin-left:10px; color:var(--color-text); text-decoration:none;'>Back to Home</a>
            </div>
        </header>
        <div class='video-grid' style='display:grid; grid-template-columns:repeat(auto-fill, minmax(250px, 1fr)); gap:20px; padding:20px;'></div>
    `;
    const grid = mainContent.querySelector('.video-grid');
    // Assuming createVideoCard exists in app.js
    videos.forEach(v => {
        if (typeof createVideoCard === 'function') {
...
function saveImportedPlaylist(name, stringIds) {
    ...
    localStorage.setItem(newName, JSON.stringify(videos));
    alert(`Playlist "${newName}" saved to your library!`);
}
